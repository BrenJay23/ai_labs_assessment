...
def get_feature_vector(city: str, date: str = None, weather: dict = None) -> pd.DataFrame:
    """
    Build feature vector for prediction.
    - If date provided: look up exact 2010-equivalent row for city
    - If no date: use city annual average
    - weather overrides take priority over historical values
    """
    ...
    if date:
        ...
        baseline = row[_feature_cols].iloc[0] if len(row) > 0 else city_df[_feature_cols].mean()
    else:
        baseline = city_df[_feature_cols].mean()

    # Override with user-provided weather
    if weather:
...
def predict_pvout(city: str, date: str = None, weather: dict = None) -> float:
    """Predict PVOUT (kWh/kWp/day)."""
    X = get_feature_vector(city, date, weather)
    return float(_model.predict(X)[0])


def compute_yield(pvout: float, farm_area_ha: float) -> dict:
    """Apply yield formula to convert PVOUT to daily energy output."""
